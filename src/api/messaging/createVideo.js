...
module.exports = function (defaultFuncs, api, ctx) {
  function handleUpload(msg, form) {
    return new Promise((resolve, reject) => {
      ...
      for (const attachment of msg.attachment) {
        ...
        const main = defaultFuncs
          .postFormData('https://upload.facebook.com/ajax/react_composer/attachments/video/upload', ctx.jar, {
            source: 8,
      ...
      Promise.all(uploads)
        .then(res => {
          res.forEach(payload => {
            if (payload) form.input.attachments.push({ video: { id: payload.videoID } });
          });
          resolve();
        })
        .catch(reject);
  ...
  return function createVideo(msg, callback) {
    let cb;
