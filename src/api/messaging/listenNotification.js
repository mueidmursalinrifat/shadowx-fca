...
function format(res, globalCallback) {
  ...
  for (let index of res.notifications_page.edges) {
    ...
    if (checkMinutes(new Date(timestamp * 1000), new Date()) <= 1)
      globalCallback(null, {
        id: index.node.notif.id,
        noti_id: index.node.notif.notif_id,
...
module.exports = function (defaultFuncs, api, ctx) {
  ...
  return function listenNotification(callback) {
    ...
    if (typeof callback == 'function')
      ...
      globalCallback = (error, message) =>
        error ? Message.emit('error', error) : Message.emit('message', message);
