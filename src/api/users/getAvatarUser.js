'use strict';

const utils = require("../../utils/sifuShim");

module.exports = function (defaultFuncs, api, ctx) {
  function handleAvatar(userIDs, height, width) {
    var uploads = [];

    userIDs.map(function (v) {
      var mainPromise = defaultFuncs
        .get(
          `https://graph.facebook.com/${v}/picture?height=${height}&width=${width}&redirect=false&access_token=` + ctx.access_token,
          ctx.jar
        )
        .then(utils.parseAndCheckLogin(ctx, defaultFuncs))
        .then(function (res) {
          return { userID: v, url: res.data.url };
        });
      uploads.push(mainPromise);
    });

    return Promise.all(uploads).then(function (res) {
      return res.reduce(function (Obj, { userID, url }) {
        Obj[userID] = url;
        return Obj;
      }, {});
    });
  }

  return function getAvatarUser(userIDs, size, callback) {
    var cb;
    var rtPromise = new Promise(function (resolve, reject) {
      cb = (err, res) => res ? resolve(res) : reject(err);
    });

    if (size === undefined) size = [1500, 1500];
    if (typeof size == 'string' || typeof size == 'number') size = [size, size];
    else if (Array.isArray(size) && size.length == 1) size = [size[0], size[0]];

    if (typeof size == 'function') {
      callback = size;
      size = [1500, 1500];
    }
    if (typeof callback == 'function') cb = callback;
    if (!Array.isArray(userIDs)) userIDs = [userIDs];

    var [height, width] = size;

    if (!ctx.access_token) {
      utils.error('getAvatarUser', 'Cannot get access_token');
      return cb('Cannot get access_token');
    }

    handleAvatar(userIDs, height, width)
      .then(function (res) {
        return cb(null, res);
      })
      .catch(function (err) {
        utils.error('getAvatarUser', err);
        return cb(err);
      });

    return rtPromise;
  };
};
