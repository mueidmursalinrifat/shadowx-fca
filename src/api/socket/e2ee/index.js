...
/**
 * SHADOWX-FCA E2EE Bridge — Signal Protocol + Noise WebSocket
 *
 * Uses SHADOWX-FCA's own native E2EE engine bundled at ./vendor/fb-e2ee.cjs.
 * No external npm package needed — all E2EE code is owned by SHADOWX-FCA.
 *
...
function loadFBClient() {
    try {
        ...
        throw new Error(
            "SHADOWX-FCA E2EE engine failed to load.\n" +
...
class E2EEBridge {
    ...
    async _doConnect(deviceStorePath, userId) {
        ...
        // Default to .shadowx-fca/e2ee_device.json in the user's project directory
        if (!deviceStorePath) {
        ...
        // Re-use the already-loaded session from SHADOWX-FCA's cookie jar
        ...
        // Build the SHADOWX-FCA adapter that replaces fb-messenger-e2ee's internal
        ...
        const _ctx = this.ctx;
        const _fcanxApi = this.api;
        const _defaultFuncs = this.defaultFuncs;

        const _fcanxAdapter = {
            fb_dtsg: _ctx.fb_dtsg,
            getCurrentUserID: () => _fcanxApi.getCurrentUserID(),
            getAppState: () => _fcanxApi.getAppState(),
            ...
            // SHADOWX-FCA has its own separate MQTT listener so we provide a no-op stub
            // to prevent a competing second listener from being started.
            listenMqtt: function () { return; },
            stopListenMqtt: function () {},
            setOptions: function () {},
            // Delegate messaging methods to SHADOWX-FCA's real api
            sendMessage: function (msg, threadID, callback, replyToMessage) {
                return _fcanxApi.sendMessage(msg, threadID, callback, replyToMessage);
            },
            setMessageReaction: function (reaction, messageID, callback, force) {
                if (typeof _fcanxApi.setMessageReaction === "function")
                    return _fcanxApi.setMessageReaction(reaction, messageID, callback, force);
            },
            unsendMessage: function (messageID, callback) {
                if (typeof _fcanxApi.unsendMessage === "function")
                    return _fcanxApi.unsendMessage(messageID, callback);
            },
            sendTypingIndicator: function (isTyping, threadID, callback) {
                if (typeof _fcanxApi.sendTypingIndicator === "function")
                    return _fcanxApi.sendTypingIndicator(isTyping, threadID, callback);
            },
            markAsRead: function (threadID, read, callback) {
                if (typeof _fcanxApi.markAsRead === "function")
                    return _fcanxApi.markAsRead(threadID, read, callback);
            },
            muteThread: function (threadID, muteSeconds, callback) {
                if (typeof _fcanxApi.muteThread === "function")
                    return _fcanxApi.muteThread(threadID, muteSeconds, callback);
            },
            setTitle: function (newTitle, threadID, callback) {
                if (typeof _fcanxApi.setTitle === "function")
                    return _fcanxApi.setTitle(newTitle, threadID, callback);
            },
            changeGroupImage: function (image, threadID, callback) {
                if (typeof _fcanxApi.changeGroupImage === "function")
                    return _fcanxApi.changeGroupImage(image, threadID, callback);
            },
        };

        // Inject the adapter as a global so that fb-e2ee.cjs's internal
        // require("../../vendor/fca-unofficial") login() intercepts it and
        // skips the real Facebook login, returning SHADOWX-FCA's session instead.
        global._fcanxE2EEAdapter = _fcanxAdapter;

        logger.info("E2EE", "Bootstrapping auth via vendored E2EE engine (SHADOWX-FCA session)...");
        let resolvedUserId;
        try {
        ...
        if (this.client.controller) {
            this.client.controller.api = _fcanxAdapter;
        }
     ...
     * Attachments on non-E2EE threads fall back to api.sendMessage (SHADOWX-FCA's own).
     */
